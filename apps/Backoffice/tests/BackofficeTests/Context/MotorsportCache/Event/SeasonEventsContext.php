...
use Behat\Step\Then;
use Behat\Step\When;
use Kishlin\Apps\Backoffice\MotorsportCache\Event\SyncSeasonEventsCommandUsingSymfony;
...
final class SeasonEventsContext extends BackofficeContext
{
    ...
    {
        try {
            self::cacheDatabase()->loadFixture("motorsport.event.seasonEvents.{$this->format($seasonEvents)}");
        } catch (Throwable $e) {
