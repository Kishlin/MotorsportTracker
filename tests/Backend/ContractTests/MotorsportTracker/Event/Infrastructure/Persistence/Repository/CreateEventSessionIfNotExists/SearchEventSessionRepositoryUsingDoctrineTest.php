...
use Kishlin\Backend\MotorsportTracker\Event\Infrastructure\Persistence\Repository\CreateEventSessionIfNotExists\SearchEventSessionRepositoryUsingDoctrine;
use Kishlin\Backend\Shared\Domain\ValueObject\UuidValueObject;
use Kishlin\Tests\Backend\Tools\Test\Contract\CoreRepositoryContractTestCase;

/**
 * @internal
 * @covers \Kishlin\Backend\MotorsportTracker\Event\Infrastructure\Persistence\Repository\CreateEventSessionIfNotExists\SearchEventSessionRepositoryUsingDoctrine
 */
final class SearchEventSessionRepositoryUsingDoctrineTest extends CoreRepositoryContractTestCase
{
    public function testItReturnsTheIdWhenItExists(): void
    {
        ...
        $repository = new SearchEventSessionRepositoryUsingDoctrine(self::connection());

        $eventId = $this->fixtureId('motorsport.event.event.dutchGrandPrix2022');
        $typeId  = $this->fixtureId('motorsport.event.sessionType.race');

        self::assertSame(
            self::fixtureId('motorsport.event.eventSession.dutchGrandPrix2022Race'),
            $repository->search(new UuidValueObject($eventId), new UuidValueObject($typeId))?->value(),
        );
    }
    ...
    public function testItReturnsNullWhenItDoesNotExist(): void
    {
        ...
        self::assertNull($repository->search(new UuidValueObject($eventId), new UuidValueObject($typeId)));
    }
}
