...
final class FixturesSaverForUseCaseTests extends FixtureSaver
{
    ...
    protected function saveAggregateRoot(AggregateRoot $aggregateRoot): void
    {
        ...
        if ($aggregateRoot instanceof EventCached) {
            $this->testServiceContainer->eventCachedRepositorySpy()->save($aggregateRoot);

            return;
        }

        if ($aggregateRoot instanceof EventResultsByRace) {
            $this->testServiceContainer->eventResultsByRaceRepositorySpy()->save($aggregateRoot);
        ...
        if ($aggregateRoot instanceof RaceLap) {
            $this->testServiceContainer->raceLapRepositorySpy()->save($aggregateRoot);
